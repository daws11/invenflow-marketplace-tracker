...
//
// Sharing decision (C3a): Option B — duplicated from
// `apps/web/src/lib/invenflow-client.ts`. `packages/shared` is not a
// declared dep of either app today (no entry in package.json, no symlink in
// node_modules); Option A would require `pnpm install` which the C3a brief
// forbids. The web copy stays canonical; if either changes, mirror the
// other (and the canonical `packages/shared` copy when it lands as a real
// dep).
//
...
function parseRetryAfterMs(headerVal: unknown): number | null {
  ...
  // Spec: integer seconds OR HTTP-date. We support seconds first then HTTP-date.
